...
export default function LikeButton(props: LikeProps) {
    ...
    const getCurrentUser = async () => {
        if (localStorage.getItem('token') !== null && localStorage.getItem('token') !== "") {
            await axios.get(`/verifytoken?token=${localStorage.getItem('token')}`)
                .then(async (response) => {
    ...
    return (
        <div>
            <Button variant={likeStatus} onClick={(e) => updateLike(e, likeCount, likeStatus, props.p_id, currentUser)}>
